...
export class StripeService {
  ...
  private initializeStripe() {
    const apiKey = this.dynamicConfigService.getStripeApiKey();
  ...
  async updateStripeApiKey(apiKey: string) {
    this.dynamicConfigService.setStripeApiKey(apiKey);
    this.initializeStripe();
  }
  ...
  async createSession(): Promise<Stripe.Checkout.Session> {
  ...
  async addInvoiceItem(
    ...
    return await this.stripe.invoiceItems.create({ customer, invoice, price, quantity, description, currency }).catch(error => {
      Logger.error(error);
  ...
  async retrieveBalance(): Promise<Stripe.ApiList<any>> {
    return await this.stripe.balanceTransactions.list().catch(error => {
      Logger.error(error);
