...
import { AddCodes, ImportCodes, ReuseCodes, UpdateCode, UpdateRequestedCode } from '../../application';
import { ETA_CODE_USECASE_PROVIDERS } from '../../domain';
import { AddETACodesDto, CodeDto, ReuseETACodesDto, UpdateETACodeDto, UpdateRequestedETACodeDto } from '../dtos';
...
export class EtaCodeController {
  constructor(
    ...
    @Inject(ETA_CODE_USECASE_PROVIDERS.UPDATE_REQUESTED_CODE)
    private readonly updateRequestedCodeUsecase: UpdateRequestedCode,
  ) {}

  @Get('organizations/:organizationId/codes/eta/import')
  ...
  async importCodes(@Param('organizationId') organizationId: string, @Param('authority') authority: Authority): Promise<CodeDto[]> {
    return this.importCodesUsecase.execute(Authority.ETA, organizationId);
  }
  ...
  @Put('organizations/:organizationId/codes/:codeId/eta/requested')
  @ApiOperation({ summary: 'Update an existing ETA-specific Code.' })
  //TODO: ADD BELOW
  // @ApiParam({
  //   name: 'organizationId',
  ...
  //   description: 'Returns the updated ETA Code.',
  // })
  async updateRequestedEtaCode(
    @Param('organizationId') organizationId: string,
    @Param('codeId') codeId: string,
    @Body() dto: UpdateRequestedETACodeDto,
  ): Promise<CodeDto> {
    return this.updateRequestedCodeUsecase.execute({ ...dto, id: codeId, organizationId, authority: Authority.ETA });
  }
  ...
  //TODO: ADD ENDPOINT FOR CRON JOB FOR QUEUE PENDING CODES
  async fetchQueuePendingCodesAndUpdate() {
    // return this.codeService.handleItemCodeQueueUsecase();
  }
}
